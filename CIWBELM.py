# -*- coding: utf-8 -*-
"""
Created on Mon Mar 28 23:24:42 2016
...
class CIWBELM(BELM):
    ...
    def __init__(self, InSize, HidSize, OutSize, OPIUM_type, genWeights_type, \
                 data, label, act_fun = tanh, H=1, stochastic=False,\
                 binaryTrain=False, binaryTest=True):
        
        print ">>> Initialing CIW-BELM model <<<"
        smtic = time.time() 
        ...
        smtoc = time.time()
        print "CIW-BELM Initialization complete, time cost: %3.2f seconds"%(smtoc-smtic) 
    
    def trainModel(self, train_data, train_label):
        train_data = self.__normalizeTest(train_data)
        super(CIWBELM, self).trainModel(train_data, train_label)
        
    def testModel(self, test_data, test_label):
        test_data = self.__normalizeTest(test_data)
        super(CIWBELM, self).testModel(test_data, test_label)
